...
server <- function(input, output, session) { 
    ...
    observeEvent(vals$data_raw, {
        ...
        strains <- strains %>% mutate(tp1_single_mult = ifelse(tp1_cluster_size_2>2,"Belonging to multi strain clusters in TP1","Belonging to single strain clusters in TP1"))
        strains <- strains %>% mutate(tp2_single_mult = ifelse(tp2_cluster_size_2>2,"Belonging to multi strain clusters in TP2","Belonging to single strain clusters in TP2"))
    ...
    clusters_sh1 <- SharedData$new(clusters_r1, key = ~key, group = "clusters")
    clusters_sh2 <- SharedData$new(clusters_r2, key = ~key, group = "clusters")
        ...
        plot_mapbox(mode = 'scattermapbox') %>%
            add_markers(data = clusters_sh1,
                        ...
                        marker =list(size = ~log10(cluster_size_2)*10,
                                     opacity = 0.6),
                        hovertext = ~paste(tp1_cluster)) %>%
            add_markers(data = clusters_sh2, 
                        x = ~avg_longitude,
                        y = ~avg_latitude,
                        color= ~I("#a6cee3"),
                        name = "Timepoint 2",
    ...
    output$ridgeplot <- renderPlotly({
        ggplotly(ggplot() +
                     geom_freqpoly(strains_sh1, mapping=aes(x = strain_date, group = tp1_cluster, col = as.factor(timepoint)), stat = "count" )+ 
                     geom_freqpoly(strains_sh2, mapping=aes(x = strain_date, group = tp1_cluster, col = as.factor(timepoint)), stat = "count" )+ 
                     scale_color_manual(values = c("#1F78C8", "#a6cee3"), name  = "Timepoint") +
                     labs(x="Date", y = "Count") + 
                     theme_bw() + 
    ...
    # single vs multistrain clusters
    output$singlemultclust <- renderPlotly({
        
        tp1 <- plot_ly() %>%
            add_trace(data = strains %>% subset(timepoint==1),
                      type = "histogram",
                      x = ~strain_date,
                      color = ~tp1_single_mult,
                      colors = c("#a6cee3","#1F78C8"),
                      name = ~tp1_single_mult,
                      xbins = list(size = 86400000.0)) %>%
            layout(barmode = "stack")
        
        tp2 <- plot_ly() %>%
            add_trace(data = strains %>% subset(timepoint==2),
                      type = "histogram",
                      x = ~strain_date,
                      color = ~tp2_single_mult,
                      colors = c( "#a6cee3"),
                      name = ~tp2_single_mult,
                      xbins = list(size = 86400000.0)) %>%
            layout(barmode = "stack")
        
        subplot(tp1, tp2, shareY=T) %>%
            layout(annotations = list(
                list(x = 0.2 , y = 1, text = "Timepoint 1", showarrow = F, xref='paper', yref='paper'),
                list(x = 0.8 , y = 1, text = "Timepoint 2", showarrow = F, xref='paper', yref='paper'))
            )  
